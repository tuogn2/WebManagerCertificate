    ...
    const fetchEnrollmentData = async (selectedMonth, selectedYear, type) => {
        ...
        try {
            const endpoint = type === 'course'
                ? `${API_BASE_URL}/statsOr/top-enrollments-course?month=${selectedMonth}&year=${selectedYear}&organizationId=${user.id || user._id}`
                : `${API_BASE_URL}/statsOr/top-enrollments-bundle?month=${selectedMonth}&year=${selectedYear}&organizationId=${user.id || user._id}`;
